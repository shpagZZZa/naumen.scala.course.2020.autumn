...
object Exercises {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortWith((first, second) => getWeight(first._2) < getWeight(second._2)).map(_._1)
    }

    def getWeight(data: (Int, Double)): Double = {
        data._2 * 4 * Math.PI * Math.pow(data._1, 3) / 3
    }
}
